...
// Problem 1 average donation amount
let average = 0;
let anotherVariable = 0;
average = runners.reduce(function(acc, runner){
  return acc + runner.donation;
}, 0);
anotherVariable = average / runners.length;
console.log(anotherVariable);

// Problem 2 email list
let emailList = [];
runners.forEach(function(runner){
...
// Problem 3 large donations
let largeDonations = [];
runners.filter(function(runner){
 if(runner.donation > 200) {
   largeDonations.push(`${runner.first_name} ${runner.donation}`);
 }
});
